...
Tutte queste informazioni, come già esplicitato nel capitolo precedente e nei paragrafi precedenti, sono salvate all'interno di uno ``state server'', il quale opera su delle tabelle persistenti (a meno di un riavvio totale del un nodo), create e fornite dal supervisor del nodo, al fine di non aver problematiche in caso di fallimenti.
...
Questo modulo, come accennato nei paragrafi precedenti, si occupa dello ``smistamento'' delle azioni ricevute dai nodi vicini (tramite il modulo \textit{communication\_in}) e di gestire la logica di attivazione delle regole che possono attivarsi.
...
Per processare le varie azioni inserite nella coda viene utilizzato un segnale apposito interno al modulo (\textit{handle\_next\_action}): nel momento in cui si sta inserire una nuova azione nella coda si controlla se questa sia vuota, e, in caso affermativo, il modulo si invia il messaggio in questione. Questo meccanismo permette d'avere sempre e solo un segnale di questo tipo. Nel momento in cui si finisce di processare un'azione estratta dalla testa della coda, si procede, eventualmente, ad eseguire la prossima azione ``rinviandosi'' il messaggio di prima, fino all'esaurimento della stessa.
...
Le nuove regole attivate devono essere processate prima di eseguire una nuova azione: per questo motivo verranno inserite in una coda prioritaria (\textit{priority\_queue}). Questa coda viene eseguita in egual maniera rispetto alla \textit{action\_queue}, ma ha la precedenza rispetto a questa.

Al termine di queste operazioni si verifiche se posso eseguire una delle regole nella lista \textit{on\_timer\_hold}: se è possibile vengono estratte le regole interessate e verranno inserite in testa nell'\textit{action\_queue}.

\subsection{Propagazione delle regole}
I passi eseguiti per la propoagazione delle regole sono simili a quanto detto nel paragrafo precedente: riassumeremo velocemente l'iter delle azioni compiute.
...
In questo caso, diversamente dalle azioni interne(?), il clock della regola può modificare le variabili, ed apporrà il valore di clock della regola: in caso in cui più regole vadano ad utilizzare la medesima variabile allo stesso clock (per modifiche o condizioni) è una situazione contemplata.
